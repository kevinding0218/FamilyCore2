...
@Component({
  selector: 'app-vegetable-form-query',
  templateUrl: './vegetable-form.component.html'
})
export class VegetableFormQueryComponent implements OnInit {
  ...
  private setVegetable(vege: any) {
    ...
    this.vegetable.addedById = vege.addedById;
    this.vegetable.updatedOn = vege.updatedOn;
    this.vegetable.lastUpdatedById = vege.lastUpdatedById;
    this.oldNote = this.vegetable.note = vege.note;
  }

  submit() {
    if (this.vegetable.keyValuePairInfo.id) {
      this.vegetable.lastUpdatedById = 2;
